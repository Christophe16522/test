...
public partial class CMSPages_GetFile : GetFilePage
{
    ...
    public bool IsLiveSite
    {
        get
        {
            if (mIsLiveSite == null)
            {
                mIsLiveSite = ViewMode.IsLiveSite();
            }
    ...
    protected void SendFile(CMSOutputAttachment attachment)
    {
        ...
        if ((attachment != null) && attachment.IsValid)
        {
            ...
            if (useClientCache && AllowCache && AllowClientCache && ETagsMatch(etag, attachment.LastModified))
            {
                ...
                SetTimeStamps(attachment);

                RespondNotModified(etag, !attachment.IsSecured);
                return;
            }
    ...
    /// </summary>
    /// <param name="attachment">Output file data</param>
    private void SetTimeStamps(CMSOutputAttachment attachment)
    {
        DateTime expires = DateTime.Now;

        // Send last modified header to allow client caching
        Response.Cache.SetLastModified(attachment.LastModified);
        ...
        if (node == null)
        {
            ...
            using (var cs = new CachedSection<TreeNode>(ref node, CacheMinutes, !allowLatestVersion, null, "getfilenodebyaliaspath|", CurrentSiteName, CacheHelper.GetBaseCacheKey(false, true), currentAliasPath))
            {
                if (cs.LoadData)
                {
                    ...
                    if (node == null)
                    {
                        DataSet ds = DocumentAliasInfoProvider.GetDocumentAliases("AliasURLPath='" + SqlHelper.GetSafeQueryString(currentAliasPath, false) + "'", "AliasCulture DESC", 1, "AliasNodeID, AliasCulture");
    ...
    protected CMSCacheDependency GetNodeDependency(TreeNode node)
    {
    ...
    protected bool LoggingActivityEnabled(CMSOutputAttachment attachment)
    {
        ...
        if (ActivitySettingsHelper.ActivitiesEnabledAndModuleLoaded(CurrentSiteName) && ActivitySettingsHelper.ActivitiesEnabledForThisUser(MembershipContext.AuthenticatedUser))
        {
            if (attachment.Attachment != null)
            {
                // Get allowed extensions (if not specified log everything)
                bool doLog = true;
                string tracked = SettingsKeyInfoProvider.GetStringValue(CurrentSiteName + ".CMSActivityTrackedExtensions");
    ...
    private bool LogFileDownload(CMSOutputAttachment attachment)
    {
        ...
        return (((attachment.FileNode.DocumentLogVisitActivity == true)
             ...
             && ValidationHelper.GetBoolean(attachment.FileNode.GetInheritedValue("DocumentLogVisitActivity", SiteInfoProvider.CombineWithDefaultCulture(CurrentSiteName)), false)));
    }
