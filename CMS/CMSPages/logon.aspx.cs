    ...
    private void btnItem_Click(object sender, EventArgs e)
    {
        ...
        if (IsForgottenPassword)
        {
            ...
            if ((txtUserName != null) && !string.IsNullOrEmpty(txtUserName.Text.Trim()))
            {
                ...
                result = AuthenticationHelper.ForgottenEmailRequest(txtUserName.Text.Trim(), siteName, "Logon page", SettingsKeyInfoProvider.GetStringValue(siteName + ".CMSSendPasswordEmailsFrom"), null, AuthenticationHelper.GetResetPasswordUrl(siteName), out success, returnUrl);
                if (!success)
                {
                    DisplayError(result);
                }
