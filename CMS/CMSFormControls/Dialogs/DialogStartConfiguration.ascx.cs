...
public partial class CMSFormControls_Dialogs_DialogStartConfiguration : FormEngineUserControl
{
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        communityLoaded = ModuleManager.IsModuleLoaded(ModuleName.COMMUNITY);
        mediaLoaded = ModuleManager.IsModuleLoaded(ModuleName.MEDIALIBRARY);
    ...
    private void LoadSiteGroups(string siteName)
    {
        if (communityLoaded && mediaLoaded)
        {
            ...
            if (siteName != null)
            {
                drpGroups.WhereCondition = "GroupSiteID IN (SELECT SiteID FROM CMS_Site WHERE SiteName = '" + SqlHelper.GetSafeQueryString(siteName, false) + "')";
    ...
    private void LoadSiteLibraries(string siteName)
    {
        if (mediaLoaded)
        {
            ...
            if (siteName != null)
            {
                drpSiteLibraries.WhereCondition = "LibrarySiteID IN (SELECT SiteID FROM CMS_Site WHERE SiteName = '" + SqlHelper.GetSafeQueryString(siteName, false) + "')";
    ...
    private void LoadGroupLibraries(string siteName, string groupName, bool addNone = false)
    {
        if (mediaLoaded && communityLoaded)
        {
            ...
            {
                drpGroupLibraries.WhereCondition = String.Format("LibraryGroupID IN (SELECT GroupID FROM Community_Group WHERE GroupSiteID IN (SELECT SiteID FROM CMS_Site WHERE SiteName = '{0}') AND GroupName = N'{1}')", SqlHelper.GetSafeQueryString(siteName, false), SqlHelper.GetSafeQueryString(groupName, false));
