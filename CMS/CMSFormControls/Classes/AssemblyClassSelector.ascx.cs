...
public partial class CMSFormControls_Classes_AssemblyClassSelector : FormEngineUserControl, ICallbackEventHandler
{
    ...
    private void SetupControl()
    {
        ...
        sb.AppendFormat(@"
jQuery(function() {{
    jQuery('#{0}_txtCombo').on('autocompleteselect', function(event, ui) {{
        var assemblyName = ui.item.value;
        {1}
    }})

    jQuery('#{0}_txtCombo').on('autocompletesearch', function(event, ui) {{        
        var assemblyName = jQuery('#{0}_txtCombo').val();
        {1}
    }})

    function fillClassInput(returnedArrayAsJSON) {{
        var returnedArray = JSON.parse(returnedArrayAsJSON);
        jQuery('#{2}_txtCombo').val(returnedArray[0]);
        jQuery('#{2}_txtCombo').autocomplete('option', 'source', returnedArray);
    }}
}});
