            ...
            if ((Form != null) && (!Form.IsInsertMode))
            {
                ...
                if ((obj != null))
                {
                    ...
                    if (info != null)
                    {
                        ...
                        string targetObjectColumn = (info.ColumnNames.IndexOf(obj.IDColumn) == 0) ? info.ColumnNames[1] : info.ColumnNames[0];

                        DataSet ds = info.GetData(null, obj.IDColumn + " = " + obj.ObjectID + " AND " + targetObjectColumn + " = " + targetObjectID, null, 1, null, false);
        ...
        if ((bindObjects || !Form.IsInsertMode) && Visible)
        {
            ...
            if (obj != null)
            {
                try
                {
                    ...
                    if ((bindingObj != null) && (bindingObj.ColumnNames.Count >= 2) && (bindingObj.ContainsColumn(obj.IDColumn)) && (targetObjectID > 0))
                    {
                        // Select proper column for target object
                        string targetObjectColumn = (bindingObj.ColumnNames.IndexOf(obj.IDColumn) == 0) ? bindingObj.ColumnNames[1] : bindingObj.ColumnNames[0];

                        bindingObj.SetValue(obj.IDColumn, obj.ObjectID);
