...
public partial class CMSFormControls_System_ObjectCategorySelector : FormEngineUserControl
{
    ...
    private GroupedDataSource mGroupedCategories = null;
    private GroupedDataSource mGroupedObjects = null;
    private DataSet mCategories = null;
    ...
    private GroupedDataSource GroupedObjects
    {
        get
        {
            ...
            {
                string objColumns = ObjectInfo.Generalized.CategoryIDColumn + "," + ObjectInfo.Generalized.DisplayNameColumn + "," + ObjectInfo.Generalized.IDColumn;
                DataSet dsObjects = ObjectInfo.Generalized.GetData(null, ObjectWhereCondition, ObjectInfo.Generalized.TypeInfo.DefaultOrderBy, 0, objColumns, false);

                if (!DataHelper.DataSourceIsEmpty(dsObjects))
                {
                    mGroupedObjects = new GroupedDataSource(dsObjects, ObjectInfo.Generalized.CategoryIDColumn);
                }
            }
    ...
    private String CategoryIDColumn
    {
        get
        {
            return String.IsNullOrEmpty(CategoryColumn) ? CategoryInfo.Generalized.CategoryIDColumn : CategoryColumn;
        }
    }
    ...
    private void ValidateSettings()
    {
        ...
        else if ((CategoryIDColumn == String.Empty) && ((ObjectInfo.Generalized.ObjectCategory == null) || (CategoryInfo.Generalized.CategoryIDColumn == unknownColumn)))
        {
            throw new Exception("[ObjectCategorySelector]: The object type '" + ObjectType + "' has no hierarchical category object.");
        }
        else if (ShowObjects && ((ObjectInfo.Generalized.DisplayNameColumn == unknownColumn) || (ObjectInfo.Generalized.IDColumn == unknownColumn)))
        {
            throw new Exception("[ObjectCategorySelector]: 'Show objects' setting cannot be applied for binding object type '" + ObjectType + "'.");
        }
        else if ((CategoryIDColumn == String.Empty) && DataHelper.DataSourceIsEmpty(GroupedCategories))
        {
    ...
    private void FillDropDown(int parentID, int level)
    {
        if (GroupedCategories != null)
        {
            ...
            if (gCategory != null)
            {
                ...
                {
                    if (!ExcludedItems.Contains(ValidationHelper.GetInteger(drCategory[CategoryInfo.Generalized.IDColumn], 0)))
                    {
                        string name = ResHelper.LocalizeString(drCategory[CategoryInfo.Generalized.DisplayNameColumn].ToString());
                        int categoryId = ValidationHelper.GetInteger(drCategory[CategoryInfo.Generalized.IDColumn], 0);

                        List<DataRowView> objectGroup = ((!ShowEmptyCategories || ShowObjects) && GroupedObjects != null) ? GroupedObjects.GetGroup(drCategory[CategoryInfo.Generalized.IDColumn]) : null;
                            ...
                            if (ShowObjects && !DataHelper.DataSourceIsEmpty(objectGroup))
                            {
                                if (objectGroup != null)
                                {
                                    ...
                                    foreach (DataRowView childObject in objectGroup)
                                    {
                                        ...
                                        string objValue = childObject[ObjectInfo.Generalized.IDColumn].ToString();
