...
public partial class CMSFormControls_System_UIElementPropertiesEditor : FormEngineUserControl
{
    ...
    private void GenerateNewRow()
    {
        ...
        imgOK.OnClientClick = "if (validateCustomProperties($j('#" + txtNewName.ClientID + "').val(),null))" + ControlsHelper.GetPostBackEventReference(tblData, "add") + " ;return false";
...
function disableEdit() {
    $j('.customproperties_imgedit').prop('disabled', true);
    $j('.customproperties_imgedit').attr('src', disabledEdit);

    $j('.customproperties_imgdelete').prop('disabled', true);
    $j('.customproperties_imgdelete').attr('src', disabledDelete);

    $j('.customproperties_customadd').prop('disabled', true);
    $j('.customproperties_customadd').attr('src', disabledAdd);
}

function allowEdit() {
    $j('.customproperties_imgedit').prop('disabled', false);
    $j('.customproperties_imgedit').attr('src', enabledEdit);

    $j('.customproperties_imgdelete').prop('disabled', false);
    $j('.customproperties_imgdelete').attr('src', enabledDelete);

    $j('.customproperties_customadd').prop('disabled', false);
    $j('.customproperties_customadd').attr('src', enabledAdd);
}

function disableEditButtons(key) {
    disableEdit();

    $j('#' + key + '_del').addClass('hidden');
    $j('#' + key + '_undo').removeClass('hidden');
}

function enableEditButtons(key) {
    allowEdit();

    $j('#' + key + '_del').removeClass('hidden');
    $j('#' + key + '_undo').addClass('hidden');
}

function undoCustomChanges(prefix, key) {
    var tk = $j('#' + prefix + '_tk' + key);
    var tv = $j('#' + prefix + '_tv' + key);
    var sk = $j('#' + prefix + '_sk' + key);
    var sv = $j('#' + prefix + '_sv' + key);
    tk.val(sk.text());
    tv.val(sv.text());
...
function displayDefault(prefix, key) {
    $j('#' + prefix + '_' + key + '_edit').removeClass('hidden');
    $j('#' + prefix + '_' + key + '_del').removeClass('hidden');

    $j('#' + prefix + '_' + key + '_ok').addClass('hidden');
    $j('#' + prefix + '_' + key + '_undo').addClass('hidden');

    allowEdit();
}

function displayEdit(prefix, key) {
    $j('#' + prefix + '_' + key + '_edit').addClass('hidden');
    $j('#' + prefix + '_' + key + '_del').addClass('hidden');

    $j('#' + prefix + '_' + key + '_ok').removeClass('hidden');
    $j('#' + prefix + '_' + key + '_undo').removeClass('hidden');
...
function addNewRow(prefix) {
    ...
    var nr = $j('#' + prefix + '_utk_newkey');
    var nv = $j('#' + prefix + '_utv_newvalue');
    ...
    $j('#' + prefix + '_newok').removeClass('hidden');
    $j('#' + prefix + '_newcancel').removeClass('hidden');
    $j('#' + prefix + '_add').addClass('hidden');
...
function cancelNewRow(prefix) {
    ...
    $j('#' + prefix + '_newok').addClass('hidden');
    $j('#' + prefix + '_newcancel').addClass('hidden');
    $j('#' + prefix + '_add').removeClass('hidden');
...
function clearError() {
    $j('.EditingFormErrorLabel').text('');
}

function validateCustomProperties(key, currentVal) {
 var regexp =  new RegExp('" + keyRegExp + @"');
 var match  = regexp.test(key);
 var le = $j('#" + lblError.ClientID + @"');
 if (!match) {
    le.text('" + ScriptHelper.EscapeHTMLString(GetString("uicontext.editor.currentinvalidkey")) + @"');
    return false;
 }
 
 var l = $j('.CustomEditorKeyClass').filter(function() {
        return $j(this).text() === key;
 });

 if (l.length > 0) {    
     if (key != currentVal) {
       le.text('" + ScriptHelper.EscapeHTMLString(GetString("uicontext.editor.keyalreadyexists")) + @"');
       return false;
     }
 }
