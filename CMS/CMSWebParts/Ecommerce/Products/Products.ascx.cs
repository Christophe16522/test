...
public partial class CMSWebParts_Ecommerce_Products_Products : CMSAbstractWebPart
{
    ...
    private string GetColumnWhereCondition(string column, string condition)
    {
        return string.Format("({0}) OR (({1} IS NULL) AND ({2}))", condition, column, GetParentProductWhereCondition(condition));
    }
    ...
    private string GetParentProductWhereCondition(string condition)
    {
        return "(SKUParentSKUID IS NOT NULL) AND (SKUParentSKUID IN (SELECT SKUID FROM COM_SKU WHERE " + condition + "))";
    }
