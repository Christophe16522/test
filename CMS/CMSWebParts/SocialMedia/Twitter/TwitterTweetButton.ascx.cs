    ...
    private void RenderPluginHTML5()
    {
        ...
        switch (Type.ToLowerCSafe())
        {
            case "share":
                ...
                {
                    sb.Append(" data-text=\"", HTMLHelper.HTMLEncode(TweetText), "\"");
                }
                break;
        ...
        {
            sb.Append(" data-via=\"", HTMLHelper.HTMLEncode(Via), "\"");
        }

        if (!string.IsNullOrEmpty(RelatedAccounts))
        {
            sb.Append(" data-related=\"", HTMLHelper.HTMLEncode(RelatedAccounts), "\"");
        }

        if (!string.IsNullOrEmpty(Hashtags))
        {
            sb.Append(" data-hashtags=\"", HTMLHelper.HTMLEncode(Hashtags), "\"");
        }
        sb.Append("></a>");
