...
public partial class CMSWebParts_TaggingCategories_CategoryList : CMSAbstractWebPart
{
        ...
        if (!StopProcessing)
        {
            ...
            if ((DisplayGlobalCategories) || (DisplayCustomCategories) || (DisplaySiteCategories))
            {
                ...
                else
                {
                    ...
                    {
                        rptCategoryList.ItemTemplate = CMSDataProperties.LoadTransformation(this, "[]");
                    }
                    else
                    {
                        rptCategoryList.ItemTemplate = CMSDataProperties.LoadTransformation(this, TransformationName);
                    }
    ...
    protected string GetCompleteWhereCondition()
    {
        ...
        {
            comleteWhere = TreeProvider.GetCompleteWhereCondition(SiteContext.CurrentSiteName, AliasPath, CultureCode, CombineWithDefaultCulture, null, SelectOnlyPublished, MaxRelativeLevel);
            mUseCompleteWhere = true;

            // Add complete where condition
            where = SqlHelper.AddWhereCondition(where, comleteWhere);
        }
        ...
        if (!string.IsNullOrEmpty(StartingCategory))
        {
            ...
            where = SqlHelper.AddWhereCondition(where, "(CategoryUserID IS NOT NULL) OR (CategoryIDPath LIKE N'" + SqlHelper.GetSafeQueryString(startingIdPath) + "/%')");
        }
