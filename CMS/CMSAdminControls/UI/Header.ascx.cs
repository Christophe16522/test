...
");
    ...
    private void CheckTrial()
    {
        ...
        if (LicenseHelper.ApplicationExpires != DateTime.MinValue)
        {
            ...
            // Application expires
            if (CMSVersion.SUFFIX.Contains("BETA"))
            {
            ...
            else
            {
                if (appExpiration.Ticks <= 0)
                {
                    info = string.Format(GetString("Preview.AppExpired"), CMSVersion.SUFFIX);
                }
                else
                {
                    info = string.Format(GetString("Preview.AppExpiresIn"), CMSVersion.SUFFIX, GetExpirationString(appExpiration.Days));
                }
            }
        }
        ...
        else if (DataHelper.GetNotEmpty(RequestContext.CurrentDomain, string.Empty) != string.Empty)
        {
            ...
            {
                TimeSpan expiration = lki.ExpirationDateReal.Subtract(DateTime.Now);
