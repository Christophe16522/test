            ...
            case SelectionModeEnum.MultipleButton:
                {
                    ...
                    {
                        script = string.Format("function SelectItems(items,hash) {{ wopener.{0}(items.replace(/^{1}+|{1}+$/g, ''),hash); CloseDialog(); }}", callbackMethod, valuesSeparator);
                    }
                }
                break;
...
function ProcessItem(chkbox, hash, changeChecked, getHash) {
    ...
    {
        checkHash = '|' + items.replace(/", valuesSeparator, @"/g,';') + '#' + hash;
    }
    if (getHash) {
...
function SelectAllItems(checkbox, hash) {
                ...
                case SelectionModeEnum.MultipleButton:
                    {
                        ...
                        chkAll.Attributes.Add("onclick", string.Format("SelectAllItems(this,'{0}')", ValidationHelper.GetHashString(sb.ToString().Trim(new [] { ';' }))));
    ...
    {
        if ((iObjectType.TypeInfo.DisplayNameColumn != ObjectTypeInfo.COLUMN_NAME_UNKNOWN))
        {
            // Get column for display name 
            return iObjectType.TypeInfo.DisplayNameColumn;
        }
        else if ((iObjectType.TypeInfo.CodeNameColumn != ObjectTypeInfo.COLUMN_NAME_UNKNOWN))
        {
            // Get column for code name
            return iObjectType.TypeInfo.CodeNameColumn;
        }
        else if (!String.IsNullOrEmpty(displayNameFormat))
        {
    ...
    string ICallbackEventHandler.GetCallbackResult()
    {
        ...
        if (!string.IsNullOrEmpty(callbackValues))
        {
            ...
            if (values.Length == 2)
            {
                ...
                isValid = (checkValues.Length == 2) && ValidationHelper.ValidateHash(checkValues[0].Trim(new[] { ';' }), checkValues[1], true, false);
            }
