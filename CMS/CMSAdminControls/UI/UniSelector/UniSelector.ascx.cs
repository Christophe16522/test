...
public partial class CMSAdminControls_UI_UniSelector_UniSelector : UniSelector, IPostBackEventHandler, ICallbackEventHandler
{
    ...
    private readonly Hashtable mParameters = new Hashtable();
    ...
    protected override void LoadViewState(object savedState)
    {
        ...
        if (Request.Form.AllKeys.Contains(hdnHash.UniqueID))
        {
            ...
            hashFromRequest = !string.IsNullOrEmpty(hdnHash.Value);
        }
        ...
        if (hashFromRequest && !(AllowEditTextBox && (mode == SelectionModeEnum.SingleTextBox || mode == SelectionModeEnum.MultipleTextBox)))
        {
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        // Add markup priority css class and where condition
        SetPriorityCssClass(PriorityWhereCondition, PriorityCSSClass);
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        if ((mode == SelectionModeEnum.Multiple) && (gridView.HeaderRow != null))
        {
            ...
            chkAll.InputAttributes.Add("onclick", string.Format("US_SelectAllItems('{0}', '{1}', this, 'chk{2}')", UniSelectorClientID, ValuesSeparator, mCheckBoxClass));
    ...
    /// <summary>
    /// Overridden method to get a parameter value.
    /// </summary>
    /// <param name="propertyName">Property name</param>
    public override object GetValue(string propertyName)
    {
        if (mParameters.Contains(propertyName))
        {
            return mParameters[propertyName];
        }
    ...
    public override bool SetValue(string propertyName, object value)
    {
        ...
        mParameters[propertyName] = value;
    ...
    private void ValidateValue()
    {
        ...
        mHashIsValid = ValidationHelper.ValidateHash(value, hdnHash.Value, true, false);
    ...
    private void LoadObjects()
    {
        if (Object != null)
        {
            ...
            if (url != null)
            {
                ...
                mJavaScript.Append("function US_SelectionDialog_", UniSelectorClientID, "(values) { ", Page.ClientScript.GetCallbackEventReference(this, "values", "US_SelectionDialogReady_" + UniSelectorClientID, "'" + ResolveUrl(url) + "'"), "; } \n");
            }
            ...
            if (DisplayNewButton)
            {
                ...
                {
                    mJavaScript.Append("function US_NewItem_", UniSelectorClientID, "(selectedItem) {{ var url = '", ResolveUrl(newUrl), "';selectedItem = US_TrimSeparators(selectedItem, '", ValuesSeparator, @"');modalDialog(url.replace(/##ITEMID##/i, selectedItem),'NewItem', ", EditDialogWindowWidth, ", ", EditDialogWindowHeight, "); }} \n");
                }
            }
...
function US_EditItem_", UniSelectorClientID, @"(selectedItem) {
    selectedItem = US_TrimSeparators(selectedItem, '", ValuesSeparator, @"');
    if (selectedItem == '') {
...
");
    ...
    private void EnsureColumnNames()
    {
        ...
        {
            EnabledColumnName = Object.TypeInfo.EnabledColumn;
        }
    }


    /// <summary>
    /// Gets the confirmation script
    /// </summary>
    ...
    private string GetNewUrl()
    {
        ...
        if (!String.IsNullOrEmpty(NewItemPageUrl))
        {
            ...
            newUrl = URLHelper.AddParameterToUrl(NewItemPageUrl, "selectorId", UniSelectorClientID) + AdditionalUrlParameters;

            // Ensure dialog hash if not set externally
            bool hashExists;
            URLHelper.GetQueryValue(newUrl, "hash", out hashExists);
            if (!hashExists)
            {
                newUrl = UIContextHelper.AppendDialogHash(newUrl);
            }
        }
        else
        {
            // New item page defined by UIelement
            newUrl = GetUIElementPageUrl(NewItemElementName, AdditionalUrlParameters);
        }
        return newUrl;
    }
    ...
    private string GetEditUrl()
    {
        ...
        if (!String.IsNullOrEmpty(EditItemPageUrl))
        {
            ...
            editUrl = URLHelper.AddParameterToUrl(EditItemPageUrl, "selectorId", UniSelectorClientID) + AdditionalUrlParameters;

            // Ensure dialog hash if not set externally
            bool hashExists;
            URLHelper.GetQueryValue(editUrl, "hash", out hashExists);
            if (!hashExists)
            {
                editUrl = UIContextHelper.AppendDialogHash(editUrl);
            }
        }
        else
        {
            // Edit item page defined by UIelement
            editUrl = GetUIElementPageUrl(EditItemElementName, "&action=edit" + AdditionalUrlParameters);
            editUrl = URLHelper.AddParameterToUrl(editUrl, "objectid", "##ITEMID##");
        }
        return editUrl;
    }
    ...
    {
        mParameters["SelectionMode"] = SelectionMode;
        mParameters["ResourcePrefix"] = ResourcePrefix;
        mParameters["ObjectType"] = ObjectType;
        mParameters["ReturnColumnName"] = ReturnColumnName;
        mParameters["AllowEmpty"] = AllowEmpty;
        mParameters["AllowDefault"] = AllowDefault;
        mParameters["AllowAll"] = AllowAll;
        mParameters["NoneRecordValue"] = NoneRecordValue;
        mParameters["AllRecordValue"] = AllRecordValue;
        mParameters["FilterControl"] = FilterControl;
        mParameters["UseDefaultNameFilter"] = UseDefaultNameFilter;
        mParameters["WhereCondition"] = WhereCondition;
        mParameters["OrderBy"] = OrderBy;
        mParameters["ItemsPerPage"] = ItemsPerPage;
        mParameters["EmptyReplacement"] = EmptyReplacement;
        mParameters["Values"] = String.Format("{0}{1}{0}", ValuesSeparator, values);
        mParameters["DisplayNameFormat"] = DisplayNameFormat;
        mParameters["DialogGridName"] = DialogGridName;
        mParameters["AdditionalColumns"] = AdditionalColumns;
        mParameters["CallbackMethod"] = CallbackMethod;
        mParameters["AllowEditTextBox"] = AllowEditTextBox;
        mParameters["FireOnChanged"] = OnSelectionChangedAvailable();
        mParameters["DisabledItems"] = DisabledItems;
        mParameters["AddGlobalObjectSuffix"] = AddGlobalObjectSuffix;
        mParameters["AddGlobalObjectNamePrefix"] = AddGlobalObjectNamePrefix;
        mParameters["GlobalObjectSuffix"] = GlobalObjectSuffix;
        mParameters["RemoveMultipleCommas"] = RemoveMultipleCommas;
        mParameters["IsSiteManager"] = IsSiteManager;
        mParameters["FilterMode"] = GetValue("FilterMode");
        mParameters["AdditionalSearchColumns"] = AdditionalSearchColumns;
        mParameters["SiteWhereCondition"] = GetSiteWhereCondition();
        mParameters["UseTypeCondition"] = UseTypeCondition;
    ...
    private String CreateSearchResultSet(string search)
    {
        ...
        if (bi != null)
        {
            ...
            {
                String where = bi.Generalized.DisplayNameColumn + " LIKE '%" + SecurityHelper.GetSafeQueryString(search) + "%'";
    ...
    private void AppendField(string text, string value, string hash, StringBuilder sb)
    {
    ...
    /// <summary>
    /// Creates where condition based on object's display name
    /// </summary>
    /// <param name="cond">Condition term</param>
    private String CreateDisplayNameCondition(String cond)
    {
        return (Object != null) ? Object.DisplayNameColumn + " LIKE '%" + SecurityHelper.GetSafeQueryString(cond) + "%'" : null;
    }


    /// <summary>
    /// Selects first item, if no special fields are set, select frist item from data.
    /// </summary>
...
{{
    ...
    private void RegisterAutocompleteScripts()
    {
        ...
        {
            events += String.Format("$j('#{0}_txtAutocomplete').bind('onBeforeChange', function (e, value) {{{1}}});", ClientID, OnBeforeClientChanged);
        }

        if (!String.IsNullOrEmpty(OnAfterClientChanged))
        {
            events += String.Format("$j('#{0}_txtAutocomplete').bind('onAfterChange', function (e, value) {{{1}}});", ClientID, OnAfterClientChanged);
        }
...
function {0}_result (result) {{    
   $j('#{0}_txtAutocomplete').data('responseData',result);
}}
...
null), Page.ClientScript.GetPostBackEventReference(this, "selectionchanged"), AllowEmpty.ToString().ToLowerCSafe(), String.IsNullOrEmpty(mInitSet) ? "''" : mInitSet, useCallback.ToString().ToLowerCSafe(), events,
    ...
    /// </summary>
    private void LoadSpecialFields()
    {
        // Raise event
        ...
        if (AllowDefault)
        {
            SpecialFields.Insert(0, new SpecialField
            {
                ...
                Text = GetString(ResourcePrefix + ".defaultchoice|general.defaultchoice")
            });
        }
        ...
        if (AllowEmpty)
        {
            ...
            {
                Value = NoneRecordValue,
                Text = GetString(ResourcePrefix + ".empty|general.empty")
            });
        }
        ...
        if (AllowAll)
        {
            SpecialFields.Insert(0, new SpecialField
            {
                ...
                Text = GetString(ResourcePrefix + ".selectall|general.selectall")
            });
        }

        try
        {
            SpecialFields.FillItems(drpSingleSelect.Items);
        }
    ...
    private string GetUIElementPageUrl(string elementName, String additionalQuery = "")
    {
        UIElementInfo elem = UIElementInfoProvider.GetUIElementInfo(ElementResourceName, elementName);

        string query = "returnhandler=" + "US_SelectNewValue_" + UniSelectorClientID + "&returntype=" + ReturnColumnType + additionalQuery;

        return UIContextHelper.GetElementDialogUrl(elem, 0, query);
    }
    ...
    private string GetItemName(DataRow dr)
    {
        ...
        {
            itemName = ResHelper.LocalizeString(ValidationHelper.GetString(DataHelper.GetDataRowValue(dr, Object.DisplayNameColumn), String.Empty));
        }
        else
        {
            itemName = ResHelper.LocalizeString(MergeText(DisplayNameFormat, dr));
        }
        ...
        {
            if ((Object != null) && (Object.SiteIDColumn != ObjectTypeInfo.COLUMN_NAME_UNKNOWN))
            {
                itemName += (ValidationHelper.GetInteger(DataHelper.GetDataRowValue(dr, Object.SiteIDColumn), 0) > 0 ? String.Empty : " " + GlobalObjectSuffix);
            }
        }
    ...
    /// <summary>
    /// Set css class for items where condition is true.
    /// Can be used repeatedly, but the first occurrence will apply.
    /// </summary>
    /// <param name="whereCondition">Where condition to assign css class.</param>
    /// <param name="cssClass">Css class to assign.</param>
    private void SetPriorityCssClass(string whereCondition, string cssClass)
    {
        if (!String.IsNullOrEmpty(cssClass) && !String.IsNullOrEmpty(whereCondition))
        {
            prioritizeItemsWhere.Add(new KeyValuePair<string, string>(whereCondition, cssClass));
        }
    }
            ...
            case "select":
                {
                    ...
                    chkCheckbox.InputAttributes.Add("onclick", string.Format("US_ProcessItem('{0}', '{1}', this);", UniSelectorClientID, ValuesSeparator));
...
return false;", UniSelectorClientID, ValuesSeparator, mCheckBoxClass, ScriptHelper.GetString(itemID).Trim('\''));
    ...
    void ICallbackEventHandler.RaiseCallbackEvent(string eventArgument)
    {
        ...
        if (UseUniSelectorAutocomplete)
        {
            ...
            // Smart search moreitems request, add dialog where condition based on typed value
            if (eventArgument.StartsWithCSafe(US_MORE_RECORDS.ToString()))
            {
                ...
                {
                    WhereCondition = SqlHelper.AddWhereCondition(WhereCondition, CreateDisplayNameCondition(searchTerm));
                }
            }
        }
    }
