...
$j(document).on('click', 'span[name=treeNode]', function(){
    ...
    protected void RegisterJavascriptForPreselection()
    {
        if (!String.IsNullOrEmpty(SelectedNode))
        {
            ...
            String targetUrl = String.IsNullOrEmpty(StartingPage) ? "$j(node).parent().attr('href')" : "'" + StartingPage + "'";
            string script = @"
        ...
        if (itemData != null)
        {
            ...
            if (MembershipContext.AuthenticatedUser.IsAuthorizedPerUIElement(uiElement.ElementResourceID, uiElement.ElementName, ModuleAvailabilityForSiteRequired))
            {
                ...
                if (uiElement.ElementTargetURL == "@")
                {
                    ...
                    const string onClick = "onClick=\"var js = $j(this).parents('tr').find('a').attr('href');eval(js);return false; \"";
