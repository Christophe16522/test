...
public partial class CMSAdminControls_UI_UIProfiles_UIElementCheckBoxTree : CMSUserControl, ICallbackEventHandler
{
    ...
    private const string SELECT_DESELECT_LINKS = @"&nbsp;(<span class='link' onclick=""SelectAllSubelements($j(this), {0}, {1}); {2}; return false;"">{3}</span>,&nbsp;<span class='link' onclick=""DeselectAllSubelements($j(this), {0}, {1}); {2};return false;"" >{4}</span>)";
...
function DeselectAllSubelements(elem, id, hasChkBox) {{
    ...
    private void SelectDeselectAll(bool select, int parentId, bool excludeRoot)
    {
        ...
        {
            where += " AND ElementName NOT LIKE '" + SqlHelper.GetSafeQueryString(GroupPreffix, false) + "%'";
        }
