...
function InitializeEditor() {
    ...
    if (jQuery('.menu-header-item-selected').find('.js-trim-init').length && jQuery('.js-btn-crop:enabled').length) {
        window.initializeCrop = true;
    }

    jQuery('.header-inner').click(function () {
        // Initialize only if OK button for crop is enabled
        if (jQuery(this).hasClass('js-trim-init') && jQuery('.js-btn-crop:enabled').length) {
            InitCrop();
            jQuery(window)
    ...
    jQuery('.js-btn-crop-reset').click(function () {
        LockAspectRatio(false);
        DestroyCrop();
        InitCrop();
