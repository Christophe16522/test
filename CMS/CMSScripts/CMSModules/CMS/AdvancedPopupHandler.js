...
function addBackgroundClickHandler(win) {
    ...
    jQuery(topOverlaySelector).on('click', function () {
...
function removeBackgroundClickHandler() {
    var topOverlaySelector = '.ui-widget-overlay:last';
    jQuery(topOverlaySelector).unbind('click');
}
...
function advancedModal(url, name, width, height, otherParams, noWopener, forceModal, setTitle, opener) {
    ...
    if (getExistingModal(name, opener) == null) {
        ...
        // Create new frame
        var $modalFrame = jQuery(document.createElement('iframe'));
        storeModal($modalFrame);

        // Append frame to wrapping element
        var $modalWrap = jQuery('#modalWrap');
        ...
        var $modalDialog = $modalFrame.dialog({
            ...
            open: function (event) {
                ...
                jQuery(iframe).load(function () {
                    ...
                    if (containsFrame(iframe.contentDocument) && jQuery.browser.webkit) {
                        var framesCount = 0;
                        jQuery('frame', iframe.contentDocument).each(function (index, frame) {
                ...
                function containsFrame(element) {
                    return jQuery('frame', element).length !== 0;
                }
                ...
                function dialogContainsCloseButton(iframe) {
	                ...
	                if (containsFrame(iframe.contentDocument)) {
		                jQuery('frame', iframe.contentDocument).each(function(index, frame) {
		                	if (jQuery('.close-button', frame.contentDocument).length > 0) {
				                result = true;
			                }
		                });
	                } else {
	                	result = jQuery('.close-button', iframe.contentDocument).length > 0;
	                }
