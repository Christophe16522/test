define(['CMS/Core', 'CMS/EventHub', 'CMS/Application'], function (Core, hub, cmsapp) {
	'use strict';
	...
	var defaultData = {
	    ...
	    Module = function (opt) {
		    var core = new Core(opt, defaultData),
		        ctx = core.ctx,
		        data = ctx.data,
		        $toolbar = $j('#' + data.toolbarId),
		        $helpTopicsMenuItem = $j('#' + data.helpTopicsMenuItemId),
		        $helpTopicsList = $helpTopicsMenuItem.find('ul.dropdown-menu'),
		        $searchMenuItem = $j('#' + data.searchMenuItemId),
		        $searchInput = $searchMenuItem.find('input'),
		        searchUrlPattern = data.searchUrlPattern,
		        $descriptionMenuItem = $j('#' + data.descriptionMenuItemId),
		        $contextHelpBtn = $j('.js-context-help'),
		    
		        updateContextHelp = function (app, w) {
		            // Disable F1 in IE
		            $j(w).bind('help', function () { return false; });
		        ...
		        getWindowContextHelp = function (win, applicationContext) {
		    	    ...
		    	    try {
		    		    ...
		    		    if (subPagesHelpTopics.length) {
		    			    if ($j(win.document).find('iframe').length) {
			    ...
			    getLink = function (topic) {
			        return $j('<a target="_blank" href="' + topic.url + '"></a>')
					    .html(topic.name);
			    },
			    ...
			    init = function() {
			        $toolbar.appendTo($j('#' + data.toolbarPlaceholderId))
				        .removeClass('hide')
