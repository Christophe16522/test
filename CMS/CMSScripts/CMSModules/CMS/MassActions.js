 ...
 */
define(["CMS/WebFormCaller", "Underscore"], function(webFormCaller) {
    "use strict";

    // External underscore dependency
    var _ = window._,
        MassActions = function (data) {
            this.data = data;
            this.$scope = $j("#" + data.scopeDropDownClientID);
            this.$action = $j("#" + data.actionDropDownClientID);
            this.$uniGridHiddenField = $j("#" + data.selectedItemsClientID);
            this.$messagePlaceHolder = $j("#" + data.messagePlaceHolderClientID);
            this.data = data;

            var $button = $j("#" + data.buttonClientID),
                that = this;
