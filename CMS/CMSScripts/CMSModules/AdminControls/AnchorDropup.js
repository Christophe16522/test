...
define(['jQueryJScrollPane'], function () {
    'use strict';
    ...
    AnchorDropup.prototype.getDropupPadding = function() {
        return parseInt(jQuery('.anchor-dropup .dropdown-menu').css('padding-top'), 10);
    };

    // Scrolls to target anchor with offset
    AnchorDropup.prototype.smoothScroll = function(elem) {

        var target = jQuery(elem.hash);
        var offset;
        ...
        target = target.length && target || jQuery('[name=' + elem.hash.slice(1) + ']');

        if (target.length) {
            // Scroll to target with offset
            offset = target.offset().top - this.getHeaderOffset() - this.scrollOffset;
            jQuery('html, body').stop(true, false).animate({ scrollTop: offset }, 300);
            return false;
        }
    };
    ...
    AnchorDropup.prototype.getHeaderOffset = function() {
        ...
        if ((headerOffset === 0) && (headerOffsetHeight === 0)) {
            return jQuery('.cms-edit-menu').outerHeight();
        }
    ...
    AnchorDropup.prototype.initScrollableDropup = function() {
        ...
        var anchorMaxHeight = jQuery('.anchor-dropup').offset().top - this.getDropupPadding() * 2 - this.getHeaderOffset() - this.scrollOffset;
        // Backup wrapper padding
        var anchorPadding = this.getDropupPadding();
        var elem = jQuery('.anchor-dropup .dropdown-menu');
        ...
        // Fix HTML validity
        jQuery('.jspContainer').wrap('<li></li>');
        jQuery('.jspPane').wrapInner('<ul class="dropdown-menu dropdown-menu-inner-wrap"></ul>');
    };
    ...
    AnchorDropup.prototype.initQuickNavigation = function(offset) {
        ...
        jQuery('.anchor-dropup a[href^="#"]').each(function() {
        ...
        if (jQuery('.jspContainer').length === 0) {
            // Init jScrollPane
