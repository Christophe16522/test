...
function AutoCompleteExtender(codeElem, hintElem, contextElem, quickContextElem, mixedMode, editorTopOffset, editorLeftOffset, ascxMode) {
    ...
    jQuery(this.hintsDiv).parents().scroll(function(e) {
        me.positionDivs();
    });
    ...
    */
    ...
    this.positionDivs = function () {
        ...
        var scrollerPos = this.getElementPosition(scroller);
        
        // Get the maximum x position (autocomplete should not be outside the editor)
        var xMax = scrollerPos.x + scroller.offsetWidth - this.hintsDiv.offsetWidth - 20;

        // Get absolute caret position
        var caretPos = this.elem.cursorCoords();

        // Set position
        var x = Math.min(caretPos.x - 5, xMax);
        var y = caretPos.y;
        ...
        this.quickContextLeft = (x <= scrollerPos.x + scroller.offsetWidth / 2) || (scroller.offsetWidth < 300);

        var hintDown = y <= scrollerPos.y + scroller.offsetHeight / 2;
        ...
        if (this.contextDiv.style.display != 'none') {
            autocompleteHeight += this.contextDiv.offsetHeight + 5;
        }

        if (hintDown && (jQuery(document).height() < y + 25 + autocompleteHeight) && (0 <= y - 5 - autocompleteHeight)) {
            hintDown = false;
        }
        
        if (!hintDown && (0 > y - 5 - autocompleteHeight) && (jQuery(document).height() > y + 25 + autocompleteHeight)) {
            hintDown = true;
        }
    ...
    this.positionQuickContextDiv = function () {
        var pos = this.hintsDiv.getBoundingClientRect();
        var x = pos.left;
        var y = pos.top;
