...
function setUpSelector(clientID, postBack, noDataFound, initSet) {
    ...
    var tb = $j("#" + clientID + "_txtAutocomplete");

    if (tb.length > 0) {
        // Storage of selected value
        var hdnValue = $j("#" + clientID + "_hdnValue");
        // Dropdown button next to the textbox
        var btnDropDown = $j("#" + clientID + "_btnAutocomplete");
        ...
        var menuItem = tb.autocomplete({
            source:
             function (request, response) {
                 ...
                 if (initialized) {
                     ...
                     // Filter manually
                     var lowered = $j.trim(request.term.toLowerCase());
                     is = $j(is).filter(function (index) {
            ...
            select: function (event, ui) {
                ...
                switch (ui.item.Value) {
                    ...
                    default:
                        ...
                        var encoded = $j('<div />').html(ui.item.Text).text();
            ...
            open: function (event, ui) {
                ...
                var autocomplete = $j(this).autocomplete("widget");
                var w = tb.width();
                ...
                $j(this).autocomplete("widget").scrollTop(selectedIndex * 21);
            },
        ...
        $j(document).bind('mousewheel', function (e) {
            if (!widget.is(':hover')) {
                tb.autocomplete('close');
            }
        });

        $j(widget).bind('mousewheel DOMMouseScroll', function (e) {
            var scrollTo = null;
            ...
            if (scrollTo) {
                e.preventDefault();
                $j(this).scrollTop(scrollTo + $j(this).scrollTop());
            }
        });
    }
}