...
function InitGlobalVariables() {
    ...
    window.menuElem = jQuery('.cms-edit-menu');
...
function InitializeDesign() {
    ...
    if (viewElem.length > 0) {
        ...
        if (resizerArrowV.hasClass('ResizerDown')) {
            ...
            if (viewElem.height() != (mainHeight - propHeight)) {
                viewElem.height(mainHeight - propHeight - resizerLineHeight - padds);
        }
    }
    // Dialog tree height
    ...
    if (mediaLibraryBlockObj.length == 0) {
        mediaLibraryBlockObj = jQuery('.tree-actions-panel');
    }
...
function Loading(preview) {
    jQuery('.youtube-loader').remove();
    var loader;
    if (jQuery('.js-youtube-url').val() == '') {
        loader = jQuery('<div class="youtube-loader"><span>' + preview + '</span></div>');
        jQuery('div.js-youtube-preview-box').append(loader);
    }
    else {
        jQuery('div.youtube-loader').hide();
    }
