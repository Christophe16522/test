...
"
    ...
    private void EnsureEditingForm(bool modify)
    {
        ...
        {
            if ((pageTemplate != null) && !pageTemplate.Generalized.IsCheckedOutByUser(MembershipContext.AuthenticatedUser))
            {
                btnClone.Enabled = false;

                var objectType = TypeHelper.GetNiceObjectTypeName(pageTemplate.ObjectType);
