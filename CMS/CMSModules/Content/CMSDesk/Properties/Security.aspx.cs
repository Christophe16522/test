...
public partial class CMSModules_Content_CMSDesk_Properties_Security : CMSPropertiesPage
{
    ...
    {
        // If parent value isn't integer return -1
        int value = ValidationHelper.GetInteger(Node.GetInheritedValue(columnName, true), -1);
        if (value == 2)
        {
        ...
        else
        {
            ...
            return ValidationHelper.GetBoolean((value == -1) ? Node.GetInheritedValue(columnName, true) : value, false) ? GetString("General.Yes") : GetString("General.No");
        }
    }
    ...
    protected void lnkBreakWithCopy_Click(Object sender, EventArgs e)
    {
        ...
        {
            EventLogProvider.LogEvent(EventType.INFORMATION, "Content", "DOCPERMISSIONSMODIFIED", ResHelper.GetAPIString("security.documentpermissionsbreakcopy", "Inheritance of the parent document permissions have been broken. Parent document permissions have been copied."), eventUrl, DocumentManager.Tree.UserInfo.UserID, DocumentManager.Tree.UserInfo.UserName, Node.NodeID, DocumentName, ipAddress, Node.NodeSiteID);
        }
    ...
    protected void lnkRestoreInheritanceRecursively_Click(object sender, EventArgs e)
    {
        ...
        // Recursively
        ctlAsync.Parameter = MembershipContext.AuthenticatedUser;
                ...
                if (permissionsResult)
                {
                    ...
                    if (!AclInfoProvider.DoesNodeInheritPermissions(treeNode.NodeID))
                    {
                        ...
                        if (DocumentManager.Tree.LogEvents)
                        {
                            if (recursive)
                            {
                                LogContext.LogEvent(EventType.INFORMATION, "Content", "DOCPERMISSIONSMODIFIED", string.Format(ResHelper.GetAPIString("security.documentpermissionsrestoredfordoc", "Permissions of document '{0}' have been restored to the parent document permissions."), nodeAliasPath), null, user.UserID, user.UserName, treeNode.NodeID, treeNode.GetDocumentName(), ipAddress, Node.NodeSiteID, null, null, null, DateTime.Now);
                            }
                            else
                            {
                                EventLogProvider.LogEvent(EventType.INFORMATION, "Content", "DOCPERMISSIONSMODIFIED", ResHelper.GetAPIString("security.documentpermissionsrestored", "Permissions have been restored to the parent document permissions."), eventUrl, user.UserID, user.UserName, treeNode.NodeID, treeNode.GetDocumentName(), ipAddress, Node.NodeSiteID);
                            }
                        }
                    }
                    else
