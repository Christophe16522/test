...
SetSelectedMode = function (mode) {
    ...
    require(['CMS/EventHub'], function(EventHub) {
...
function ChangeSplitMode(elem) {
    ...
    try {
        var cookieValue = jQuery.cookie('CMSSplitMode');
    ...
    jQuery.cookie("CMSSplitMode", newCookieValue.toString(), { expires: 365, path: '/' });
...
function MaintainScroll(nodeId, treeId, originalScroll) {
    ...
    var actionsPanel = jQuery('.tree-actions-panel');
...
function Properties(nodeId, tab) {
    if (nodeId != 0) {
        ...
        window.PerformContentRedirect(null, 'content', nodeId, '&action=properties&tabname=' + tab);
    }
}
