...
function ChangeDevice(device) {
    ...
    $j.cookie(CMSView.RotateCookieName, null, { path: '/' });
...
var CMSView = {
    ...
    ResizeContentArea: function () {
        if (!CMSView.ViewElem.length) {
            CMSView.ViewElem = $j('#pageview');
        }
        if (!CMSView.DeviceElem.length) {
            CMSView.DeviceElem = $j('.DeviceFrame');
        }
        
        var newHeight = document.body.offsetHeight - $j('.preview-edit-panel:first').height();
