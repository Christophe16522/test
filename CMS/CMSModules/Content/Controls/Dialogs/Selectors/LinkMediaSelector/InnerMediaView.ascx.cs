    ...
    public string AllowedExtensions
    {
        get
        {
            if (mAllowedExtensions == "")
            {
                ...
                if (mAllowedExtensions == "")
                {
                    mAllowedExtensions = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".CMSUploadExtensions");
                }
            }
        ...
        switch (sourceName)
        {
            ...
            case "editlibraryui":
                ...
                if (btnAction != null)
                {
                    ...
                    {
                        btnAction.OnClientClick = String.Format("$j('#hdnFileOrigName').attr('value', {0}); SetAction('editlibraryui', {1}); RaiseHiddenPostBack(); return false;", ScriptHelper.GetString(EnsureFileName(fileName)), ScriptHelper.GetString(fileName));
                    }
                }
                break;
