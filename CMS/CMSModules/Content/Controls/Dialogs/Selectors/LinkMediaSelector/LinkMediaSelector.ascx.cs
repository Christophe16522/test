...
public partial class CMSModules_Content_Controls_Dialogs_Selectors_LinkMediaSelector_LinkMediaSelector : LinkMediaSelector
{
    ...
    /// </summary>
    private const string HELP_TOPIC_INSERT_MEDIA_LINK = "IgIvAQ";


    /// <summary>
    /// Short link to help page regarding link insertion.
    /// </summary>
    private const string HELP_TOPIC_INSERT_LINK_LINK = "PgIvAQ";
    ...
    private void SetHelp()
    {
        ...
        {
            helpTopic = UIContextHelper.GetDocumentationTopicUrl(helpTopic);
        }
    ...
    private string GetSiteWhere()
    {
        ...
        if (!String.IsNullOrEmpty(siteName))
        {
            ...
            where = SqlHelper.AddWhereCondition(where, String.Format("SiteName LIKE N'{0}'", SqlHelper.GetSafeQueryString(siteName)));
        }

        // Get only current user's sites
        if (!MembershipContext.AuthenticatedUser.IsGlobalAdministrator)
        {
            where = SqlHelper.AddWhereCondition(where, String.Format("SiteID IN (SELECT SiteID FROM CMS_UserSite WHERE UserID = {0})", MembershipContext.AuthenticatedUser.UserID));
        }
    ...
    private void SetupControls()
    {
        ...
        {
            UsePermanentUrls = SettingsKeyInfoProvider.GetBoolValue(SiteContext.CurrentSiteName + ".CMSUsePermanentURLs");
        }
        else
        {
...
", hdnAction.ClientID, hdnArgument.ClientID, refreshType, cmdName, ControlsHelper.GetPostBackEventReference(hdnButton, "")));
    ...
    private DataSet GetNodes(string searchText, string parentAliasPath, string siteName, ref int totalRecords)
    {
        ...
        {
            string searchTextSafe = SqlHelper.GetSafeQueryString(searchText, false);
...
if (wopener.parent != null) {
    ...
    private void LoadAttachmentsDataSource(string searchText)
    {
        ...
        string where = String.IsNullOrEmpty(searchText) ? "(AttachmentIsUnsorted = 1)" : SqlHelper.AddWhereCondition("(AttachmentIsUnsorted = 1)", String.Format("(AttachmentName LIKE N'%{0}%')", SqlHelper.GetSafeQueryString(searchText, false)));
    ...
    {
        string where = !String.IsNullOrEmpty(searchText) ? String.Format("(MetaFileName LIKE N'%{0}%')", SqlHelper.GetSafeQueryString(searchText, false)) : string.Empty;

        int totalRecords = -1;
        // Get metafiles
        mediaView.DataSource = MetaFileInfoProvider.GetMetaFiles(Config.MetaFileObjectID, Config.MetaFileObjectType, Config.MetaFileCategory, where, mediaView.ListViewControl.SortDirect, "MetaFileID,MetaFileObjectType,MetaFileObjectID,MetaFileGroupName,MetaFileName,MetaFileExtension,MetaFileSize,MetaFileMimeType,MetaFileImageWidth,MetaFileImageHeight,MetaFileGUID,MetaFileLastModified,MetaFileSiteID,MetaFileTitle,MetaFileDescription", -1, mediaView.CurrentOffset, mediaView.CurrentPageSize, ref totalRecords);
        mediaView.TotalRecords = totalRecords;
    }
