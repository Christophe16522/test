    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (TreeNode != null)
            {
                ...
                {
                    where = SqlHelper.AddWhereCondition(where, "RelationshipNameID IN (SELECT RelationshipNameID FROM CMS_RelationshipName WHERE RelationshipName = N'" + SqlHelper.GetSafeQueryString(RelationshipName, false) + "')");
                }
                ...
                if (!AllowSwitchSides)
                {
                    if (DefaultSide)
                    {
                        where = SqlHelper.AddWhereCondition(where, "RightNodeID = " + nodeId);
                    }
                    else
                    {
                        where = SqlHelper.AddWhereCondition(where, "LeftNodeID = " + nodeId);
                    }
                }
                else
                {
                    where = SqlHelper.AddWhereCondition(where, "(LeftNodeID = " + nodeId + ") OR (RightNodeID = " + nodeId + ")");
                }
