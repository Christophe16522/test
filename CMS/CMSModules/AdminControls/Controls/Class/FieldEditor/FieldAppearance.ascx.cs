...
public partial class CMSModules_AdminControls_Controls_Class_FieldEditor_FieldAppearance : CMSUserControl
{
    ...
    public bool Save()
    {
        if (ffi != null)
        {
            ffi.SetPropertyValue(FormFieldPropertyEnum.FieldCaption, ValidationHelper.GetString(txtFieldCaption.Value, String.Empty), txtFieldCaption.IsMacro);
            ffi.SetPropertyValue(FormFieldPropertyEnum.FieldDescription, ValidationHelper.GetString(txtDescription.Value, String.Empty), txtDescription.IsMacro);
            ffi.SetPropertyValue(FormFieldPropertyEnum.ExplanationText, ValidationHelper.GetString(txtExplanationText.Value, String.Empty), txtExplanationText.IsMacro);

            if (ShowFieldVisibility)
            {
                ffi.AllowUserToChangeVisibility = ChangeVisibility;
                ffi.Visibility = Visibility;
                ffi.VisibilityControl = VisibilityDDL;
            }

            if (ShowInheritanceSettings)
            {
                ffi.Inheritable = ControlInheritable;
            }
