...
jQuery('.pnl-edit #form-builder-properties .PanelProperties input, .pnl-edit #form-builder-properties .PanelProperties textarea').on('input propertychange', FormBuilder.setSaveSettingsTimeout);
jQuery('.pnl-edit #form-builder-properties .PanelProperties input, .pnl-edit #form-builder-properties .PanelProperties textarea').on('change', FormBuilder.saveSettings);

// This is workaround for jQueryUI bug http://bugs.jqueryui.com/ticket/7941
// Clicking on sortable doesnt trigger blur event.
jQuery('.EditingFormCategoryFields').mousedown(function () {
try {
    jQuery('.settings-content input, .settings-content textarea').blur();
} catch (err) {}
});
