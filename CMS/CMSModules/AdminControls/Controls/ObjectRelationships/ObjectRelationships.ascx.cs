...
public partial class CMSModules_AdminControls_Controls_ObjectRelationships_ObjectRelationships : CMSUserControl
{
    ...
    protected void RefreshNewSiteSelection()
    {
        if (RelatedObject != null)
        {
            if (RelatedObject.SiteIDColumn != ObjectTypeInfo.COLUMN_NAME_UNKNOWN)
            {
            ...
            if (RelatedObject != null)
            {
                ...
                if (RelatedObject.SiteIDColumn != ObjectTypeInfo.COLUMN_NAME_UNKNOWN)
                {
                    ...
                    if (siteSelector.HasData)
                    {
                        ...
                        if (selectedSiteId >= 0)
                        {
                            string siteQuery = new DataQuery(RelatedObject.ObjectType, null)
                                                    .Column(RelatedObject.IDColumn)
                                                    .Where(SqlHelper.GetSiteIDWhereCondition(RelatedObject.SiteIDColumn, selectedSiteId))
                                                    .QueryText;
    ...
    private void DisplayAvailableItems()
    {
        if (RelatedObject != null)
        {
            ...
            if (RelatedObject.SiteIDColumn != ObjectTypeInfo.COLUMN_NAME_UNKNOWN)
            {
                int selectedSiteId = selectedSiteId = ValidationHelper.GetInteger(siteSelector.Value, -1);
