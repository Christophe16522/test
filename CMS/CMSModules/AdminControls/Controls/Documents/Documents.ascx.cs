...
public partial class CMSModules_AdminControls_Controls_Documents_Documents : CMSAdminEditControl
{
    ...
    private void SetupGridColumns()
    {
        ...
        documentName.Width = (ListingType == ListingTypeEnum.All) ? "80%" : "100%";
    ...
    private void SetupGridFilterWhereCondition()
    {
        ...
        switch (ListingType)
        {
            ...
            case ListingTypeEnum.PendingDocuments:
                where = "DocumentWorkflowStepID IN (SELECT StepID FROM CMS_WorkflowStep WHERE " + WorkflowStepInfoProvider.GetWorkflowPendingStepsWhereCondition(currentUserInfo, SiteName) + ")";
                break;
        ...
        {
            gridElem.WhereCondition = SqlHelper.AddWhereCondition(gridElem.WhereCondition, ((ListingType == ListingTypeEnum.All) ? SOURCE_DOCUMENTNAMEPATH : SOURCE_NODEALIASPATH) + " LIKE N'" + MacroResolver.ResolveCurrentPath(Path).Replace("'", "''") + "'");
        }
        ...
        {
            gridElem.WhereCondition = SqlHelper.AddWhereCondition(gridElem.WhereCondition, SOURCE_DOCUMENTNAME + " LIKE N'%" + SqlHelper.GetSafeQueryString(DocumentName, false) + "%'");
        }
    }
