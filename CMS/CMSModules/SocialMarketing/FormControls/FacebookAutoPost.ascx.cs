...
public partial class CMSModules_SocialMarketing_FormControls_FacebookAutoPost : FormEngineUserControl
{
    ...
    private FacebookPostInfo PostInfo
    {
        get
        {
            if ((mPostInfo == null) && (Document != null) && (Document.DocumentGUID != Guid.Empty))
            {
                mPostInfo = FacebookPostInfoProvider.GetFacebookPostInfosByDocumentGuid(Document.DocumentGUID).ToList().FirstOrDefault(x => x.FacebookPostIsCreatedByUser);
            }
    ...
    public override bool Enabled
    {
        ...
        set
        {
            ...
            if (!value && !StopProcessing && (PostInfo == null))
            {
    ...
    /// </summary>
    /// <returns>True if feature is available, false otherwise.</returns>
    private bool IsFeatureAvailable()
    {
        return LicenseKeyInfoProvider.IsFeatureAvailable(RequestContext.CurrentDomain, FeatureEnum.SocialMarketing);
    }


    /// <summary>
    /// Indicates whether current user has read permission on Facebook posts.
    /// </summary>
    /// <returns>True if user has read permission, false otherwise.</returns>
    private bool HasUserReadPermission()
    {
        return UserInfoProvider.IsAuthorizedPerResource(ModuleName.SOCIALMARKETING, "Read", SiteContext.CurrentSiteName, MembershipContext.AuthenticatedUser, false)
            || HasUserModifyPermission();
    }


    /// <summary>
    /// Indicates whether current user has modify permission on Facebook posts.
    /// </summary>
    /// <returns>True if user has modify permission, false otherwise.</returns>
    private bool HasUserModifyPermission()
    {
        return UserInfoProvider.IsAuthorizedPerResource(ModuleName.SOCIALMARKETING, "Modify", SiteContext.CurrentSiteName, MembershipContext.AuthenticatedUser, false)
            || UserInfoProvider.IsAuthorizedPerResource(ModuleName.SOCIALMARKETING, "ModifyPosts", SiteContext.CurrentSiteName, MembershipContext.AuthenticatedUser, false);
    }
