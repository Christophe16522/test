...
public partial class CMSModules_OnlineMarketing_Controls_UI_ContentPersonalizationVariant_List : CMSAdminListControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (Node != null)
        {
            where = SqlHelper.AddWhereCondition(where, "(VariantDocumentID = " + Node.DocumentID + ") OR VariantDocumentID IS NULL");
        }

        // Display variants just for a specific zone/webpart/widget
        if (!string.IsNullOrEmpty(ZoneID))
        {
            where = SqlHelper.AddWhereCondition(where, "VariantZoneID = '" + SqlHelper.GetSafeQueryString(ZoneID, false) + "'");

            if (InstanceGUID != Guid.Empty)
            {
                // Web part/widget condition
                where = SqlHelper.AddWhereCondition(where, "VariantInstanceGUID = '" + InstanceGUID + "'");
            }
        }
