...
public partial class CMSModules_FileImport_Tools_ImportFromServer : CMSDeskPage
{
    ...
    private string GetPath()
    {
        ...
        if (!String.IsNullOrEmpty(SiteContext.CurrentSiteName))
        {
            ...
            string path = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".CMSFileImportFolder").Trim();
        ...
        else
        {
            if (gridImport.SelectedItems.Count > 0)
            {
                ...
                if (!String.IsNullOrEmpty(path))
                {
                    ...
                    ctlAsync.Parameter = new object[4] { gridImport.SelectedItems.ToArray(), path, cultureSelector.Value, MembershipContext.AuthenticatedUser };
