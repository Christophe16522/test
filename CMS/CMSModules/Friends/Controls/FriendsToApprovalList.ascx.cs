...
public partial class CMSModules_Friends_Controls_FriendsToApprovalList : CMSAdminListControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            var userId = QueryHelper.GetInteger("userId", 0);
            var currentUser = MembershipContext.AuthenticatedUser;
            friendsManagePermission = currentUser.IsAuthorizedPerResource("CMS.Friends", "Manage") || (currentUser.UserID == userId);
            ...
            if (!IsLiveSite)
            {
                ...
                headerActions.AddAction(new HeaderAction()
                {
                    ...
                    OnClientClick = "return FM_Approve_" + ClientID + "(null,'" + gridElem.SelectionHiddenField.ClientID + "','" + ApproveDialogUrl + "');",
                    Enabled = friendsManagePermission
                });
                headerActions.AddAction(new HeaderAction()
                {
                    Text = GetString("friends.friendrejectall"),
                    OnClientClick = "return FM_Reject_" + ClientID + "(null,'" + gridElem.SelectionHiddenField.ClientID + "','" + RejectDialogUrl + "');",
                    Enabled = friendsManagePermission
                });
            }
        }
