...
public partial class CMSModules_Avatars_Controls_AvatarsGallery : CMSAdminEditControl
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        string avatarScript = ScriptHelper.GetScript(
            @"function markImage(id) {
                hidden = jQuery('#" + hiddenAvatarGuid.ClientID + @"');
                if (hidden.val() != '') {
                    img = jQuery('#' + hidden.val());
    ...
    {
        return HTMLHelper.EncodeLinkReference(URLHelper.UpdateParameterInUrl(RequestContext.CurrentURL, param, ValidationHelper.GetString(number, "1")));
    }
