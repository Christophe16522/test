...
public partial class CMSModules_ContactManagement_Controls_UI_Contact_FilterSuggest : CMSUserControl
{
    ...
    public string GetWhereCondition()
    {
        ...
        if (chkEmail.Checked)
        {
            ...
            {
                string emailWhere = "ContactEmail LIKE N'%@" + SqlHelper.GetSafeQueryString(domain) + "'";
        ...
        if (chkAddress.Checked)
        {
            ...
            {
                addressWhere = SqlHelper.AddWhereCondition(addressWhere, "ContactAddress1 LIKE N'%" + SqlHelper.GetSafeQueryString(CurrentContact.ContactAddress1, false) + "%'", "OR");
            }
            if (!String.IsNullOrEmpty(CurrentContact.ContactAddress2))
            {
                addressWhere = SqlHelper.AddWhereCondition(addressWhere, "ContactAddress2 LIKE N'%" + SqlHelper.GetSafeQueryString(CurrentContact.ContactAddress2, false) + "%'", "OR");
            }
            if (!String.IsNullOrEmpty(CurrentContact.ContactCity))
            {
                addressWhere = SqlHelper.AddWhereCondition(addressWhere, "ContactCity LIKE N'%" + SqlHelper.GetSafeQueryString(CurrentContact.ContactCity, false) + "%'", "OR");
            }
            if (!String.IsNullOrEmpty(CurrentContact.ContactZIP))
            {
                addressWhere = SqlHelper.AddWhereCondition(addressWhere, "ContactZIP LIKE N'%" + SqlHelper.GetSafeQueryString(CurrentContact.ContactZIP, false) + "%'", "OR");
            }
        ...
        {
            where = SqlHelper.AddWhereCondition(where, "ContactBirthDay = '" + FormHelper.GetDateTimeValueInDBCulture(CurrentContact.ContactBirthday.ToString()) + "'");
        }
        ...
        if (chkPhone.Checked)
        {
            ...
            {
                phoneWhere = SqlHelper.AddWhereCondition(phoneWhere, "ContactBusinessPhone LIKE N'%" + SqlHelper.GetSafeQueryString(CurrentContact.ContactBusinessPhone, false) + "%'");
            }
            if (!String.IsNullOrEmpty(CurrentContact.ContactHomePhone))
            {
                phoneWhere = SqlHelper.AddWhereCondition(phoneWhere, "ContactHomePhone LIKE N'%" + SqlHelper.GetSafeQueryString(CurrentContact.ContactHomePhone, false) + "%'", "OR");
            }
            if (!String.IsNullOrEmpty(CurrentContact.ContactMobilePhone))
            {
                phoneWhere = SqlHelper.AddWhereCondition(phoneWhere, "ContactMobilePhone LIKE N'%" + SqlHelper.GetSafeQueryString(CurrentContact.ContactMobilePhone, false) + "%'", "OR");
            }
