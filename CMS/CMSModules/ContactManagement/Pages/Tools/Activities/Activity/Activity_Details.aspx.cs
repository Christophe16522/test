...
public partial class CMSModules_ContactManagement_Pages_Tools_Activities_Activity_Activity_Details : CMSModalPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!MembershipContext.AuthenticatedUser.IsAuthorizedPerResource(ModuleName.CONTACTMANAGEMENT, "ReadActivities", SiteInfoProvider.GetSiteName(ai.ActivitySiteID)))
        {
        ...
        userIsAuthorized = MembershipContext.AuthenticatedUser.IsAuthorizedPerResource(ModuleName.CONTACTMANAGEMENT, "ManageActivities", SiteInfoProvider.GetSiteName(ai.ActivitySiteID));
        if (userIsAuthorized)
        {
            Save += btnSave_Click;
    ...
    protected void RegisterScripts()
    {
        ...
        if (isSitemanager)
        {
            stamp = SettingsKeyInfoProvider.GetStringValue("CMSCMStamp");
        }
        else
        {
            stamp = SettingsKeyInfoProvider.GetStringValue(SiteContext.CurrentSiteName + ".CMSCMStamp");
        }
