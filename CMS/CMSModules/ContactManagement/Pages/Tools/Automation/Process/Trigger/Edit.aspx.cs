...
public partial class CMSModules_ContactManagement_Pages_Tools_Automation_Process_Trigger_Edit : CMSAutomationPage
{
    ...
    private void FillDdlType()
    {
        ...
        {
            string activityText = string.Format("{0} {1}", TypeHelper.GetNiceObjectTypeName(ActivityInfo.OBJECT_TYPE), GetString("ma.trigger.performed").ToLowerCSafe());
            string scoreText = string.Format("{0} {1}", TypeHelper.GetNiceObjectTypeName(ScoreInfo.OBJECT_TYPE), GetString("ma.trigger.exceeded").ToLowerCSafe());
            AddDdlTypeListItem(WorkflowTriggerTypeEnum.Creation, ActivityInfo.OBJECT_TYPE, activityText);
            AddDdlTypeListItem(WorkflowTriggerTypeEnum.Change, ScoreInfo.OBJECT_TYPE, scoreText);
        }
    }
