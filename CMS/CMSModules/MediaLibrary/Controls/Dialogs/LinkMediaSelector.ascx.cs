    ...
    private DataRow FileIsNotInDatabase(string fileName)
    {
        if (FileList == null)
        {
            string where = String.Format("FilePath LIKE N'{0}%' AND FilePath NOT LIKE N'{0}_%/%' AND FileLibraryID = {1}",
                                         Path.EnsureSlashes(SqlHelper.GetSafeQueryString(LastFolderPath, false).Replace("[", "[[]").Replace("%", "[%]")).Trim('/'),
                                         LibraryID);

            if (!string.IsNullOrEmpty(LastSearchedValue))
            {
                where = SqlHelper.AddWhereCondition(where, String.Format("((FileName LIKE N'%{0}%' {{escape '{1}'}})  OR (FileExtension LIKE N'%{0}%' {{escape '{1}'}}))",
                                                                              SqlHelper.GetSafeQueryString(LastSearchedValue, false),
                                                                              PERCENT_ESC_CHAR));
            }
    ...
    private void LoadDataSource(string searchText)
    {
        ...
        if ((FolderPath != null) && (LibraryID > 0))
        {
            ...
            if (err == string.Empty)
            {
                ...
                string filePath = (StartingPath + Path.EnsureSlashes(normFolderPath)).Trim('/').Replace("'", "''");
                ...
                if (!string.IsNullOrEmpty(searchText))
                {
                    ...
                    where = SqlHelper.AddWhereCondition(where, String.Format("(FileName LIKE N'%{0}%' {{escape '{1}'}}) OR (FileExtension LIKE N'%{0}%' {{escape '{1}'}})", SqlHelper.GetSafeQueryString(searchText, false), PERCENT_ESC_CHAR));
                }
