...
public partial class CMSModules_MediaLibrary_Controls_Dialogs_AdvancedMediaLibrarySelector : CMSUserControl
{
    ...
    private string GetSitesWhere()
    {
        ...
        switch (Sites)
        {
            case AvailableSitesEnum.OnlySingleSite:
                ...
                {
                    where = SqlHelper.AddWhereCondition(where, String.Format("SiteName LIKE N'{0}'", SqlHelper.GetSafeQueryString(SelectedSiteName)));
                }
                break;

            case AvailableSitesEnum.OnlyCurrentSite:
                where = SqlHelper.AddWhereCondition(where, String.Format("SiteName LIKE N'{0}'", SqlHelper.GetSafeQueryString(SiteContext.CurrentSiteName)));
                break;
        }
    ...
    {
        if (disableGroupSelector && ModuleManager.IsModuleLoaded(ModuleName.COMMUNITY) && (groupsSelector != null))
        {
        ...
        if (groupId > 0)
        {
            ...
            switch (GroupLibraries)
            {
                case AvailableLibrariesEnum.OnlySingleLibrary:
                    librarySelector.SiteID = SiteID;
                    result = String.Format("(LibraryName= N'{0}')", SqlHelper.GetSafeQueryString(GroupLibraryName, false));
                    break;
    ...
    private string GetGlobalLibrariesCondition()
    {
        ...
        switch (GlobalLibraries)
        {
            ...
            case AvailableLibrariesEnum.OnlySingleLibrary:
                librarySelector.SiteID = SiteID;
                return String.Format("(LibraryName= N'{0}')", SqlHelper.GetSafeQueryString(GlobalLibaryName, false));
