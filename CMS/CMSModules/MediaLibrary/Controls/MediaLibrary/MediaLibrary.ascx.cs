...
public partial class CMSModules_MediaLibrary_Controls_MediaLibrary_MediaLibrary : CMSAdminItemsControl
{
            ...
            if (err == string.Empty)
            {
                ...
                if (!IsCopyMoveLinkDialog)
                {
                    ...
                    if (DisplayOnlyImportedFiles)
                    {
                        ...
                        normFolderPath = Path.EnsureSlashes(normFolderPath).Trim('/');
                        normFolderPath = SqlHelper.GetSafeQueryString(normFolderPath, false);
                        ...
                        if (!string.IsNullOrEmpty(searchText))
                        {
                            ...
                            where += String.Format(" AND (FileName LIKE N'%{0}%' {{escape '{1}'}})", SqlHelper.GetSafeQueryString(searchText, false), PERCENT_ESC_CHAR);
                        }
                    ...
                    else
                    {
                        try
                        {
                            ...
                            if (!string.IsNullOrEmpty(searchText))
                            {
                                where = String.Format("FileName LIKE '%{0}%'", SqlHelper.GetSafeQueryString(searchText, false));
                            }
    ...
    private void HandleFolderAction(string folderPath, bool isNewFolder, bool displayNormal, bool select, bool display)
    {
        ...
        // Update menu
        menuElem.LibraryID = LibraryID;
        folderActions.LibraryFolderPath = menuElem.LibraryFolderPath = (!isNewFolder) ? LastFolderPath.Trim('/') : LastFolderPath;
        menuElem.UpdateViewMenu();
        folderActions.Update();
    ...
    private DataRow FileIsNotInDatabase(string fileName)
    {
        if (FileList == null)
        {
            string where = "FilePath LIKE N'" + Path.EnsureSlashes(SqlHelper.GetSafeQueryString(LastFolderPath, false).Replace("[", "[[]").Replace("%", "[%]")).Trim('/') +
                           "%' AND FilePath NOT LIKE N'" + Path.EnsureSlashes(SqlHelper.GetSafeQueryString(LastFolderPath, false).Replace("[", "[[]").Replace("%", "[%]")).Trim('/') +
            ...
            if (!string.IsNullOrEmpty(LastSearchedValue))
            {
                ...
                where += " AND ((FileName LIKE N'%" + SqlHelper.GetSafeQueryString(name, false) + "%') OR (FileExtension LIKE N'%" + SqlHelper.GetSafeQueryString(extension, false) + "%'))";
            }
