...
public partial class CMSModules_Ecommerce_Pages_Tools_Configuration_Departments_Department_List : CMSDepartmentsPage
{
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        UniGrid.WhereCondition = InitWhereCondition();
    ...
    /// </summary>
    private string InitWhereCondition()
    {
        string where;

        if (IsMultiStoreConfiguration)
        {
            where = "DepartmentSiteID IS NULL"; 
        }
        else
        {
            where = "DepartmentSiteID = " + SiteContext.CurrentSiteID;
            if (UseGlobalObjects)
            {
                where = SqlHelper.AddWhereCondition(where, "DepartmentSiteID IS NULL", "OR");
            }
        }
