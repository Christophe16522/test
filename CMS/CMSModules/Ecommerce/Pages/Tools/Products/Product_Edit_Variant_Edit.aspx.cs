...
public partial class CMSModules_Ecommerce_Pages_Tools_Products_Product_Edit_Variant_Edit : CMSProductsPage
{
    ...
    /// <summary>
    /// Returns starting position for attributes.
    /// </summary>
    private int GetAttributesPosition()
    {
        // Find position for attributes values - just after second form category
        int attrPos = 0;
        int catCount = 0;
        foreach (IField item in editForm.FormInformation.ItemsList)
        {
            attrPos++;
