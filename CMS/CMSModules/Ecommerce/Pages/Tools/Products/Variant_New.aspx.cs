...
public partial class CMSModules_Ecommerce_Pages_Tools_Products_Variant_New : CMSProductsPage
{
    ...
    private string CurrentWarning
    {
        get
        {
            return ValidationHelper.GetString(mWarnings["DefineWarning_" + ctlAsync.ProcessGUID], string.Empty);
        }
        set
        {
            mWarnings["DefineWarning_" + ctlAsync.ProcessGUID] = value;
        }
    }
    ...
    protected override void OnLoadComplete(EventArgs e)
    {
        ...
        if (!URLHelper.IsPostback())
        {
            ...
            {
                ShowWarning(HTMLHelper.HTMLEncode(QueryHelper.GetSQLSafeText("warning", string.Empty)));
            }
        }
            ...
            case "save":
                ...
                if ((NewCategories.Count > 0) || (ExistingCategories.Count > 0))
                {
                    ...
                    var selectedRows = from row in mGeneratedVariants.Tables[0].AsEnumerable()
                                       where VariantGrid.SelectedItems.Contains(
                    ...
                    if (((!mVariantsToGenerate.Any()) && (ExistingCategories.Count > 0) && (NewCategories.Count == 0)) || (VariantGrid.SelectedItems.Count == 0))
                    {
    ...
    private void SetupVariants(bool regenerate = true)
    {
        ...
        List<string> existingItems = ExistingItems;
    ...
    private List<string> GetExistingItems()
    {
        if (!DataHelper.DataSourceIsEmpty(mGeneratedVariants))
        {
            List<string> result = (from row in mGeneratedVariants.Tables[0].AsEnumerable()
    ...
    private void SetupAsyncControl()
    {
        ...
        ctlAsync.OnRequestLog += (s, e) => { ctlAsync.Log = CurrentLog.Log; };
        ctlAsync.OnCancel += ctlAsync_OnCancel;

        // Asynchronous content configuration
        titleElemAsync.TitleText = GetString("com.variants.defining");
        if (!RequestHelper.IsCallback())
        {
