...
",
    ...
    private string GetWhereCondition()
    {
        ...
        {
            where = SqlHelper.AddWhereCondition(where, "SKUID NOT IN (SELECT NodeSKUID FROM View_CMS_Tree_Joined WHERE NodeSKUID IS NOT NULL AND NodeSiteID = " + SiteContext.CurrentSiteID + ")");
        }
        ...
        {
            where = SqlHelper.AddWhereCondition(where, "(SKUDepartmentID IS NULL) OR SKUDepartmentID IN (SELECT DepartmentID FROM COM_UserDepartment WHERE UserID=" + cui.UserID + ")");
        }

        // Reflect "Allow global products" setting
        string siteWhere = "SKUSiteID = " + SiteContext.CurrentSiteID;
        if (AllowGlobalObjects)
        {
            siteWhere += " OR SKUSiteID IS NULL";
        }

        return SqlHelper.AddWhereCondition(where, siteWhere);
    }
    ...
    private string GetDocumentWhereCondition()
    {
        ...
        if (docList.Node != null)
        {
            ...
            path = SqlHelper.GetSafeQueryString(path.TrimEnd('/'));
            string pathCondition = string.Format("NodeAliasPath LIKE N'{0}/%'", path);
            // Combine where conditions
            return SqlHelper.AddWhereCondition(where, pathCondition);
        }
