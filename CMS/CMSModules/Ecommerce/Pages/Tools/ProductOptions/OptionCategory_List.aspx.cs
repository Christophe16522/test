...
public partial class CMSModules_Ecommerce_Pages_Tools_ProductOptions_OptionCategory_List : CMSProductOptionCategoriesPage
{
    ...
    {
        OptionCategoryGrid.WhereCondition = InitWhereCondition();
    }
        ...
        switch (actionName.ToLowerCSafe())
        {
            ...
            case "delete":
                ...
                if (!DataHelper.DataSourceIsEmpty(options))
                {
                    ...
                    {
                        if (SKUInfoProvider.CheckDependencies(ValidationHelper.GetInteger(option["SKUID"], 0)))
                        {
                            // Show error message
                            ShowError(GetString("Ecommerce.DeleteDisabled"));
    ...
    {
        var where = "CategorySiteID = " + SiteContext.CurrentSiteID;
