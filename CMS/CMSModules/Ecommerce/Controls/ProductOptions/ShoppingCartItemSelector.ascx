<%@ Control Language="C#" AutoEventWireup="true" Inherits="CMSModules_Ecommerce_Controls_ProductOptions_ShoppingCartItemSelector"
    CodeFile="ShoppingCartItemSelector.ascx.cs" %>
<%@ Register TagPrefix="cms" TagName="DonationProperties" Src="~/CMSModules/Ecommerce/Controls/ProductOptions/DonationProperties.ascx" %>


<cms:CMSUpdatePanel ID="upnlAjax" runat="server" class="updateproduct" >
    <ContentTemplate>
        <asp:Panel ID="pnlContainer" runat="server" CssClass="CartItemSelectorContainer">
            <asp:Panel ID="pnlSelectors" runat="server" CssClass="CartItemSelector">
                <%-- Donation properties --%>
                <cms:DonationProperties runat="server" ID="donationProperties" Visible="false" StopProcessing="true"
                    ShowDonationUnits="false"  />
                <%-- Here come product options --%>
            </asp:Panel>
            <%-- Price --%>
            <asp:Panel ID="pnlPrice" runat="server" CssClass="TotalPriceContainer" EnableViewState="false">
                <asp:Label ID="lblPrice" runat="server" CssClass="TotalPriceLabel" EnableViewState="false" />
                <asp:Label ID="lblPriceValue" runat="server" CssClass="TotalPrice" EnableViewState="false" />
            </asp:Panel>
            <asp:Panel ID="pnlButton" runat="server" CssClass="AddToCartContainer" EnableViewState="false">
                <%-- Add to wishlist --%>
                <asp:ImageButton ID="btnWishlist" runat="server" Visible="false" CssClass="AddToWishlistImageButton" />
                <asp:LinkButton ID="lnkWishlist" runat="server" Visible="false" CssClass="AddToWishlistLink" />
                <%-- Units --%>
                <cms:LocalizedLabel ID="lblUnits" runat="server" Visible="false" ResourceString="ecommerce.shoppingcartcontent.skuunits" />
                <ul class="XtDtlQte">
                    <li> 
                 <%--  <asp:LinkButton ID="test1" runat="server" OnClick="arrowDown"> <asp:ImageButton ID="down" runat="server"
                       ImageUrl="~/App_Themes/Servranx/images/ArrowQteL.gif"  width="7" height="12" /></asp:LinkButton>--%>
                       <%-- <input type="image" src="~/App_Themes/Servranx/images/ArrowQteL.gif" width="7" height="12"
                            alt="left" />--%>
                        <asp:Button ID="Button1" runat="server" Text="<" onclick="Button1_Click" width="7" height="12" CssClass="btnleft"/>
                    </li>
                    <li><span>
                        <cms:CMSTextBox ID="txtUnits" runat="server" Visible="false" CssClass="AddToCartTextBox"
                            MaxLength="2" ontextchanged="txtUnits_TextChanged" />
                        <ajaxToolkit:FilteredTextBoxExtender ID="FilteredTextBoxExtenderUnits" runat="server" TargetControlID="txtUnits" FilterType="Numbers">
                        </ajaxToolkit:FilteredTextBoxExtender>
                        <%-- <input type="text" name="quantite" value="0" />--%>
                        <b>Qté</b> </span></li>
                    <li  >
                        <%--<input id ="up" type="image" src="~/App_Themes/Servranx/images/ArrowQteR.gif" width="7" height="12"
                            alt="right" />--%>
                            <%--<asp:ImageButton ID="up" runat="server"
                       ImageUrl="~/App_Themes/Servranx/images/ArrowQteR.gif" OnClick="arrowUp" width="7" height="12" CausesValidation="false"/>--%>
                        <asp:Button ID="Button2" runat="server" Text=">" onclick="Button2_Click" width="7" height="12" CssClass="btnright"/>
                    </li>
                </ul>
                <%-- Add to cart controls --%>
                <cms:CMSButton ID="btnAdd" runat="server" Visible="false" CssClass="SubmitButton AddToCartButton" />
                <asp:ImageButton ID="btnAddImage" runat="server" Visible="true" CssClass="AddToCartImageButton BtnPanierDtl"
                    OnClick="btnAddImage_Click" AlternateText=" " ToolTip="Ajouter au panier" ImageUrl="~/App_Themes/Servranx/images/vide.png" CausesValidation="false"/>
               
                    
                    
                        <asp:LinkButton ID="lnkAdd" runat="server" Visible="false" CssClass="AddToCartLink BtnPanierDtl" onclick="lnkAdd_Click"/>
                 
               
            </asp:Panel>
        </asp:Panel>
        <asp:Literal ID="ltlScript" runat="server" EnableViewState="false" />
    </ContentTemplate>
</cms:CMSUpdatePanel>
<%-- --%>
