...
public partial class CMSModules_Ecommerce_Controls_Filters_ProductFilter : CMSAbstractDataFilterControl
{
    ...
    public override string SiteName
    {
        ...
        set
        {
            ...
            statusSelector.ReflectGlobalProductsUse = true;
        }
    }
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        PageHelper.CurrentPage.LoadComplete += CurrentPage_LoadComplete;
    }
    ...
    private void FilterData()
    {
        ...
        {
            where = SqlHelper.AddWhereCondition(where, "SKUPublicStatusID = " + statusSelector.SelectedID);
        }

        if (manufacturerSelector.SelectedID > 0)
        {
            where = SqlHelper.AddWhereCondition(where, "SKUManufacturerID = " + manufacturerSelector.SelectedID);
        }

        if (chkStock.Checked)
        {
            where = SqlHelper.AddWhereCondition(where, "SKUTrackInventory IS NULL " +
                                                       " OR SKUAvailableItems > 0" +
                                                       " OR SKUID IN (SELECT SKUParentSKUID FROM COM_SKU WHERE (SKUAvailableItems > 0))");
        }

        if (!string.IsNullOrEmpty(txtSearch.Text))
        {
            where = SqlHelper.AddWhereCondition(where, String.Format("SKUName LIKE N'%{0}%'", SqlHelper.GetSafeQueryString(txtSearch.Text, false)));
        }
