...
public partial class CMSModules_Ecommerce_Controls_Filters_OrderFilter : CMSAbstractDataFilterControl
{
    ...
    private string GetFilterWhereCondition()
    {
        ...
        {
            condition = SqlHelper.AddWhereCondition(condition, String.Format("OrderStatusID = {0}", statusSelector.SelectedID));
        }

        // Order site
        if (FilterSiteID > 0)
        {
            condition = SqlHelper.AddWhereCondition(condition, String.Format("OrderSiteID = {0}", FilterSiteID));
        }
        ...
        if (filterOrderId != String.Empty)
        {
            ...
            condition = SqlHelper.AddWhereCondition(condition, String.Format("OrderID = {0} OR OrderInvoiceNumber LIKE N'%{1}%'", orderId, SqlHelper.GetSafeQueryString(filterOrderId, false)));
        }
        ...
        if (CustomerFilterEnabled)
        {
            ...
            {
                condition = SqlHelper.AddWhereCondition(condition, String.Format("OrderCustomerID IN (SELECT CustomerID FROM COM_Customer WHERE (CustomerFirstName LIKE N'%{0}%') OR (CustomerLastName LIKE N'%{0}%') OR (CustomerCompany LIKE N'%{0}%'))", SqlHelper.GetSafeQueryString(customerNameOrCompany, false)));
            }
        }
        ...
        {
            case "0":
                condition = SqlHelper.AddWhereCondition(condition, "ISNULL(OrderIsPaid, 0) = 0");
                break;
            case "1":
                condition = SqlHelper.AddWhereCondition(condition, "ISNULL(OrderIsPaid, 0) = 1");
                break;
        }
        ...
        if (AdvancedFilterEnabled)
        {
            ...
            {
                condition = SqlHelper.AddWhereCondition(condition, "OrderCurrencyID = " + CurrencySelector.SelectedID);
            }

            // Specific payment method was selected
            if (PaymentSelector.SelectedID > 0)
            {
                condition = SqlHelper.AddWhereCondition(condition, "OrderPaymentOptionID = " + PaymentSelector.SelectedID);
            }

            // Specific shipping option was selected
            if (ShippingSelector.SelectedID > 0)
            {
                condition = SqlHelper.AddWhereCondition(condition, "OrderShippingOptionID = " + ShippingSelector.SelectedID);
            }
            ...
            {
                condition = SqlHelper.AddWhereCondition(condition, "OrderTrackingNumber LIKE '%" + SqlHelper.GetSafeQueryString(trackingNumber) + "%'");
            }
            ...
            {
                condition = SqlHelper.AddWhereCondition(condition, "OrderDate <= " + SqlHelper.GetSqlValue(dtpCreatedTo.SelectedDateTime));
            }

            // Specific from date was selected
            if ((dtpFrom.SelectedDateTime < dtpFrom.MaxDate) && (dtpFrom.SelectedDateTime > dtpFrom.MinDate))
            {
                condition = SqlHelper.AddWhereCondition(condition, "OrderDate >= " + SqlHelper.GetSqlValue(dtpFrom.SelectedDateTime));
            }
        }
