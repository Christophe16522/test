...
public partial class CMSModules_Ecommerce_Controls_Filters_ProductVariantFilter : CMSAbstractDataFilterControl, ISimpleDataContainer
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        string script = "function Check(check) {" +
                            " $j(\"#" + pnlFilterOptions.ClientID + " :input:checkbox\").attr('checked', check);" +
                        "}";
    ...
    private string GetFilterWhereCondition()
    {
        ...
        string variantNameOrNumber = SqlHelper.GetSafeQueryString(txtVariantNameNumber.Text, false);
        ...
        foreach (KeyValuePair<int, List<int>> pair in SelectedOptionIDs)
        {
            ...
            // Option ids for current category in string separated by ","
            string idString = TextHelper.Join(",", optionIds);
            ...
            if (!string.IsNullOrEmpty(idString))
            {
                ...
                string categoryCondition = String.Format("(SKUID IN ( SELECT VariantSKUID FROM COM_VariantOption WHERE OptionSKUID IN ({0}) ))", idString);
                // Current category conditions is joined with previous where condition with AND.
                condition = SqlHelper.AddWhereCondition(condition, categoryCondition);
            }
        }
        ...
        {
            condition = SqlHelper.AddWhereCondition(condition, String.Format("(SKUNumber LIKE N'%{0}%' OR SKUName LIKE N'%{0}%')", variantNameOrNumber));
        }

        // Condition is empty -> not a single option is checked -> grid will be empty
        return string.IsNullOrEmpty(condition) ? "(1=0)" : condition;
    }
