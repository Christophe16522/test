    ...
    protected void ReloadData()
    {
        ...
        if (IsLiveSite)
        {
            if (CurrentUser.IsAuthenticated())
            {
                ...
                try
                {
                    ShippingOptionAuthenticatedID = Convert.ToInt32(ddlShippingOptionAuthenticated.SelectedValue);
                }
                catch
                {
                  //  EventLogProvider ev = new EventLogProvider();
            ...
            if (ais.Count == 1)
            {
                ...
                else
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        lblShippingAddressFullName.Text = ECommerceContext.CurrentCustomer.CustomerFirstName + " " + ECommerceContext.CurrentCustomer.CustomerLastName; //ai.AddressPersonalName;  //GetString("DearCustomer") + " " +  
                        lblShippingAddressStreet.Text = string.IsNullOrEmpty(ai2.AddressLine2) ? ai2.AddressLine1 : string.Format("{0}, {1}", ai2.AddressLine1, ai2.AddressLine2);
                        ...
                        lblShippingAddressStreet.Text = string.Format("{0} {1}", temp.ToString(), lblShippingAddressStreet.Text).Trim();
                        lblShippingAddressZipCode.Text = ai2.AddressZip;
                        lblShippingAddressCityCountry.Text = string.Format("{0}, {1}", ai2.AddressCity, MacroContext.CurrentResolver.ResolveMacros(CountryInfoProvider.GetCountryInfo(ai2.AddressCountryID).CountryDisplayName));
