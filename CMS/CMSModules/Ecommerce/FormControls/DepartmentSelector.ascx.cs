...
public partial class CMSModules_Ecommerce_FormControls_DepartmentSelector : SiteSeparatedObjectSelector
{
    ...
    protected override void InitSelector()
    {
        ...
        if (AddAllMyRecord)
        {
            uniSelector.SpecialFields.Add(new SpecialField { Text = GetString("product_list.allmydepartments"), Value = String.Empty });
        }

        if (AddWithoutDepartmentRecord)
        {
            uniSelector.SpecialFields.Add(new SpecialField { Text = GetString("general.empty"), Value = "-5" });
        }
