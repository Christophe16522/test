...
public partial class CMSModules_Ecommerce_FormControls_SelectProductImage : FormEngineUserControl
{
    ...
    protected override void OnLoad(EventArgs e)
    {
        ...
        if (SiteID == 0)
        {
            allowedExtensions = SettingsKeyInfoProvider.GetStringValue("CMSUploadExtensions");
        }
        else if (SiteID > 0)
        {
            allowedExtensions = SettingsKeyInfoProvider.GetStringValue(SiteInfoProvider.GetSiteName(SiteID) + ".CMSUploadExtensions");
        }
