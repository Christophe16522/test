...
public partial class CMSModules_Ecommerce_FormControls_CustomerSelector : FormEngineUserControl
{
    ...
    public void InitSelector()
    {
        ...
        {
            where = SqlHelper.AddWhereCondition(where, "CustomerUserID IN (SELECT UserID FROM CMS_UserSite WHERE SiteID = " + SiteID + ")", "OR");
        }

        // Add anonymous customers
        if (DisplayAnonymousCustomers)
        {
            where = SqlHelper.AddWhereCondition(where, "(CustomerSiteID = " + SiteID + ") AND (CustomerUserID IS NULL)", "OR");
        }

        // Filter out only enabled items
        if (DisplayOnlyEnabled)
        {
            where = SqlHelper.AddWhereCondition(where, "CustomerEnabled = 1");
        }
        ...
        {
            where = SqlHelper.AddWhereCondition(where, "CustomerID IN (" + additionalList + ")", "OR");
        }

        // Selected value must be on the list
        if (CustomerID > 0)
        {
            where = SqlHelper.AddWhereCondition(where, "CustomerID = " + CustomerID, "OR");
        }
