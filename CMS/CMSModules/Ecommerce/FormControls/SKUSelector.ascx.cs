...
public partial class CMSModules_Ecommerce_FormControls_SKUSelector : FormEngineUserControl
{
    ...
    public void ReloadData()
    {
        ...
        else
        {
            ...
            {
                where = SqlHelper.AddWhereCondition(where, "SKUOptionCategoryID IS NULL AND SKUSiteID IS NULL", "OR");
            }

            // Add site specific products
            if (DisplaySiteProducts)
            {
                where = SqlHelper.AddWhereCondition(where, "SKUOptionCategoryID IS NULL AND SKUSiteID = " + SiteID, "OR");
            }
        }
        ...
        {
            where = SqlHelper.AddWhereCondition(where, String.Format("SKUProductType <> '{0}'", SKUProductTypeEnum.Product.ToStringRepresentation()));
        }

        // Exclude memberships if needed
        if (!DisplayMemberships)
        {
            where = SqlHelper.AddWhereCondition(where, String.Format("SKUProductType <> '{0}'", SKUProductTypeEnum.Membership.ToStringRepresentation()));
        }

        // Exclude e-products if needed
        if (!DisplayEproducts)
        {
            where = SqlHelper.AddWhereCondition(where, String.Format("SKUProductType <> '{0}'", SKUProductTypeEnum.EProduct.ToStringRepresentation()));
        }
        ...
        {
            where = SqlHelper.AddWhereCondition(where, String.Format("SKUID NOT IN (SELECT SKUID FROM COM_SKUOptionCategory)"));
        }
        ...
        if (DisplayProductOptions && (ProductOptionCategoryID <= 0))
        {
            ...
            {
                where = SqlHelper.AddWhereCondition(where, "SKUOptionCategoryID IS NOT NULL AND SKUSiteID IS NULL", "OR");
            }

            // Add site specific options
            if (DisplaySiteOptions)
            {
                where = SqlHelper.AddWhereCondition(where, "SKUOptionCategoryID IS NOT NULL AND SKUSiteID = " + SiteID, "OR");
            }
        }
        ...
        if (UserID > 0)
        {
            where = SqlHelper.AddWhereCondition(where, String.Format("(SKUDepartmentID IN (SELECT DepartmentID FROM COM_UserDepartment WHERE UserID = {0}) OR (SKUDepartmentID IS NULL))", UserID));
        }
        ...
        else
        {
            ...
            where = SqlHelper.AddWhereCondition(where, "SKUParentSKUID IS NULL");
        }

        // Add items which have to be on the list
        string additionalList = SqlHelper.GetSafeQueryString(AdditionalItems, false);

        if (!string.IsNullOrEmpty(additionalList))
        {
            where = SqlHelper.AddWhereCondition(where, "SKUID IN (" + additionalList + ")", "OR");
        }

        // Selected value must be on the list
        if (SKUID > 0)
        {
            where = SqlHelper.AddWhereCondition(where, "SKUID = " + SKUID, "OR");
        }
